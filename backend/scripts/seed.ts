...
type QuizDef = { name: string; label: string; genre: string; description: string; difficulty: string; timeLimit: number; data: Record<string, Entry> };
...
async function seed() {
  ...
  for (const quiz of quizDefinitions) {
    ...
    db.insert(schema.quizzes)
      .values({ quizName: quiz.name, genre: quiz.genre, description: quiz.description, difficulty: quiz.difficulty, timeLimit: quiz.timeLimit })
      .onConflictDoUpdate({
